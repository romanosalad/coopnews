...
async function scrapeArticle(url: string): Promise<ScrapedArticle> {
  const response = await fetch(url, {
    headers: {
      "User-Agent": "BriefingCoBot/0.1 (+https://briefing.co)"
    }
  });
        ...
        },
        {
          ...
          content: JSON.stringify({
            expected_schema: {
              ...
              body_markdown:
                "6 to 9 short paragraphs in Brazilian Portuguese, rewritten as an original Briefing.Co article. MUST include >=2 H2 sections (## Section title) for any article >600 words. NO bullet lists. NO copied lead. NO source link inside body. Max 4 sentences per paragraph.",
...
function composeArtDirectionBrief(refined: RefinedStory) {
  ...
  return [
    "Editorial cover image for Briefing.Co, a Brazilian cooperative-marketing intelligence magazine.",
